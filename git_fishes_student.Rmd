...
author: "Nicole Lehmann u6956268"
date: "`r Sys.Date()`"
output: 
...
[My GitHub Repository](https://github.com/nicolelehmann/Workshop6_fishes)
...
# If the code block 'answer1' is changed to echo=FALSE, the code block is removed from the rendered html. 
```
...
data_clean <- select(data_clean, -...1)
...
## The previous version of the Rmarkdown file is displayed on the left-hand side and the latest version is displayed on the right-hand side, with the lines visually matched up. The red highlights and green highlights indicate lines which have had changes made in them, with the red showing the previous version and the green showing the current version. 
```
...
data_clean %>% filter(species == "chromis"|species ==  "lemon"|species == "acantho") %>% ggplot(mapping = aes(x = species, y = activity, colour = treatment)) + geom_violin() + geom_jitter()
...
```{r, answer7}
## GitHub Desktop indicated that there was a conflicted file. When this was opened, the conflict was indicated in the format above. I decided that I liked my code changes better, so I deleted my partners code and the conflict markers. 
```
